...
const { EVENT } = protocol_js_1.JANODE;
/**
...
const connect = (config = {}, key = null) => {
    logger_js_1.default.info(`${LOG_NS} creating new connection`);
    ...
    if (!key)
        logger_js_1.default.verbose(`${LOG_NS} omitted server key, falling back to the first server in configuration`);
    const server_raw_conf = janus_server_list[index];
    if (!server_raw_conf) {
        const error = new Error(`server configuration not defined for server #${key || index}`);
        logger_js_1.default.error(`${LOG_NS} ${error.message}`);
        throw error;
    }
    const server_conf = new configuration_js_1.default(server_raw_conf);
    logger_js_1.default.verbose(`${LOG_NS} creating connection with server configuration ${JSON.stringify(server_conf)}`);
    const janus_connection = new connection_js_1.default(server_conf);
