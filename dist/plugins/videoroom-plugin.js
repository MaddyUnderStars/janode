 ...
 * @hideconstructor
 */
class VideoRoomHandle extends handle_js_1.default {
    /**
    ...
    handleMessage(janus_message) {
        ...
        if (plugindata && plugindata.data && plugindata.data.videoroom) {
            ...
            if (jsep && typeof jsep.e2ee === 'boolean')
                janode_event.data.e2ee = jsep.e2ee;
            /* Add room information if available */
            if (room)
                janode_event.data.room = room;
            ...
            switch (videoroom) {
                ...
                case 'joined':
                    ...
                    janode_event.data.publishers = message_data.publishers.map(({ id, display, talking, audio_codec, video_codec, simulcast, streams }) => {
                        ...
                        if (typeof talking !== 'undefined')
                            pub.talking = talking;
                        if (typeof audio_codec !== 'undefined')
                            pub.audiocodec = audio_codec;
                        if (typeof video_codec !== 'undefined')
                            pub.videocodec = video_codec;
                        if (typeof simulcast !== 'undefined')
                ...
                case 'stopped-talking':
                    ...
                    /* [multistream] */
                    if (typeof message_data.mid !== 'undefined')
                ...
                case 'event':
                    ...
                    if (message_data.joining) {
                        ...
                        if (message_data.joining.display)
                            janode_event.data.display = message_data.joining.display;
                        break;
                    }
                    ...
                    if (typeof message_data.leaving !== 'undefined') {
                        ...
                        if (message_data.reason)
                            janode_event.data.reason = message_data.reason;
                        break;
                    }
    ...
    joinPublisher({ room, feed, audio, video, data, bitrate, record, filename, display, token, pin, descriptions }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof feed === 'string' || typeof feed === 'number')
                body.id = feed;
            ...
            const response = yield this.message(body);
    ...
    joinConfigurePublisher({ room, feed, audio, video, data, bitrate, record, filename, display, token, pin, e2ee, descriptions, jsep }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof pin === 'string')
                body.pin = pin;
            if (typeof e2ee === 'boolean' && jsep)
                jsep.e2ee = e2ee;
            /* [multistream] */
            if (descriptions && Array.isArray(descriptions))
                body.descriptions = descriptions;
            const response = yield this.message(body, jsep).catch(e => {
    ...
    configure({ audio, video, data, bitrate, record, filename, display, restart, update, streams, descriptions, sc_substream_layer, sc_substream_fallback_ms, sc_temporal_layers, e2ee, jsep }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (streams && Array.isArray(streams)) {
                ...
                if (typeof data === 'boolean')
                    body.data = data;
                if (typeof sc_substream_layer === 'number')
                    body.substream = sc_substream_layer;
                if (typeof sc_substream_fallback_ms === 'number')
                    body.fallback = 1000 * sc_substream_fallback_ms;
                if (typeof sc_temporal_layers === 'number')
                    body.temporal = sc_temporal_layers;
            }
            if (typeof bitrate === 'number')
                body.bitrate = bitrate;
            if (typeof record === 'boolean')
            ...
            if (event === PLUGIN_EVENT.CONFIGURED && evtdata.configured === 'ok') {
                if (body.display)
                    evtdata.display = body.display;
                if (typeof body.request === 'boolean')
                    evtdata.restart = body.restart;
                if (typeof body.update === 'boolean')
                    evtdata.update = body.update;
                return evtdata;
            }
            const error = new Error(`unexpected response to ${body.request} request`);
            throw (error);
    ...
    joinSubscriber({ room, feed, audio, video, data, private_id, sc_substream_layer, sc_substream_fallback_ms, sc_temporal_layers, streams, autoupdate, use_msid, token }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof use_msid === 'boolean')
                body.use_msid = use_msid;
            const response = yield this.message(body);
            const { event, data: evtdata } = response._janode || {};
    ...
    update({ subscribe, unsubscribe }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (unsubscribe && Array.isArray(unsubscribe))
    ...
    create({ room, description, max_publishers, permanent, is_private, secret, pin, admin_key, bitrate, bitrate_cap, fir_freq, audiocodec, videocodec, talking_events, talking_level_threshold, talking_packets_threshold, require_pvtid, require_e2ee, record, rec_dir, videoorient, h264_profile, vp9_profile }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof max_publishers === 'number')
                body.publishers = max_publishers;
            ...
            if (typeof bitrate === 'number')
                body.bitrate = bitrate;
            if (typeof bitrate_cap === 'boolean')
                body.bitrate_cap = bitrate_cap;
            if (typeof fir_freq === 'number')
                body.fir_freq = fir_freq;
            ...
            if (typeof talking_level_threshold === 'number' && talking_level_threshold >= 0 && talking_level_threshold <= 127)
                body.audio_level_average = talking_level_threshold;
            if (typeof talking_packets_threshold === 'number' && talking_packets_threshold > 0)
                body.audio_active_packets = talking_packets_threshold;
            if (typeof require_pvtid === 'boolean')
                body.require_pvtid = require_pvtid;
            if (typeof require_e2ee === 'boolean')
                body.require_e2ee = require_e2ee;
            if (typeof record === 'boolean')
                body.record = record;
            if (typeof rec_dir === 'string')
                body.rec_dir = rec_dir;
            if (typeof videoorient === 'boolean')
                body.videoorient_ext = videoorient;
            if (typeof h264_profile === 'string')
                body.h264_profile = h264_profile;
            if (typeof vp9_profile === 'string')
                body.vp9_profile = vp9_profile;
            const response = yield this.message(body);
            const { event, data: evtdata } = response._janode || {};
    ...
    startForward({ room, feed, host, streams, audio_port, audio_rtcp_port, audio_ssrc, video_port, video_rtcp_port, video_ssrc, video_port_2, video_ssrc_2, video_port_3, video_ssrc_3, data_port, secret, admin_key }) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (streams && Array.isArray(streams)) {
                ...
                if (typeof audio_port === 'number')
                    body.audio_port = audio_port;
                if (typeof audio_rtcp_port === 'number')
                    body.audio_rtcp_port = audio_rtcp_port;
                if (typeof audio_ssrc === 'number')
                    body.audio_ssrc = audio_ssrc;
                if (typeof video_port === 'number')
                    body.video_port = video_port;
                if (typeof video_rtcp_port === 'number')
                    body.video_rtcp_port = video_rtcp_port;
                if (typeof video_ssrc === 'number')
                    body.video_ssrc = video_ssrc;
                if (typeof video_port_2 === 'number')
                    body.video_port_2 = video_port_2;
                if (typeof video_ssrc_2 === 'number')
                    body.video_ssrc_2 = video_ssrc_2;
                if (typeof video_port_3 === 'number')
                    body.video_port_3 = video_port_3;
                if (typeof video_ssrc_3 === 'number')
                    body.video_ssrc_3 = video_ssrc_3;
                if (typeof data_port === 'number')
                    body.data_port = data_port;
            }
            if (typeof secret === 'string')
                body.secret = secret;
            if (typeof admin_key === 'string')
