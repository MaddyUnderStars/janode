...
const debug = (0, utils_js_1.getCliArgument)('debug-tx', 'boolean', false);
/**
...
class TransactionManager {
     ...
     */
    constructor(id = (0, utils_js_1.getNumericID)()) {
        this.transactions = new Map();
        this.id = id;
        logger_js_1.default.info(`${LOG_NS} [${this.id}] creating new transaction manager (debug=${debug})`);
        this._dbgtask = null;
     ...
     */
    clear() {
        logger_js_1.default.info(`${LOG_NS} [${this.id}] clearing transaction manager`);
    ...
    set(id, transaction) {
        ...
        if (debug && !this._dbgtask) {
            this._dbgtask = setInterval(_ => {
                logger_js_1.default.info(`${LOG_NS} [${this.id}] TM DEBUG size=${this.size()}`);
            }, 5000);
        }
    }
    /**
    ...
    createTransaction(id, owner, request, done, error) {
        ...
        this.set(id, tx);
        logger_js_1.default.verbose(`${LOG_NS} [${tx.owner.id}] created new transaction ${id}, request "${tx.request}"`);
        return tx;
    }
    /**
    ...
    closeTransactionWithError(id, owner, error) {
        ...
        this.delete(id);
        tx.error(error);
        logger_js_1.default.verbose(`${LOG_NS} [${tx.owner.id}] closed with error transaction ${id}, request "${tx.request}"`);
        return tx;
    }
    /**
    ...
    closeTransactionWithSuccess(id, owner, data) {
        ...
        this.delete(id);
        tx.done(data);
        logger_js_1.default.verbose(`${LOG_NS} [${tx.owner.id}] closed with success transaction ${id}, request "${tx.request}"`);
        return tx;
    }
}
