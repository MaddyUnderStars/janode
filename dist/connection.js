...
const transport_ws_js_1 = __importDefault(require("./transport-ws.js"));
const transport_unix_js_1 = __importDefault(require("./transport-unix.js"));
const session_js_1 = __importDefault(require("./session.js"));
const tmanager_js_1 = __importDefault(require("./tmanager.js"));
/**
...
class Connection extends events_1.EventEmitter {
    ...
    constructor(server_config) {
         ...
         */
        this._tm = new tmanager_js_1.default();
        /**
         ...
         */
        this._address_iterator = (0, utils_js_1.newIterator)(this._config.getAddress());
        /**
         * A numerical identifier assigned for logging purposes.
         *
         * @type {number}
         */
        this.id = parseInt((0, utils_js_1.getNumericID)());
        /**
         ...
         */
        this._transport = {
            open: (_) => __awaiter(this, void 0, void 0, function* () { throw new Error('transport does not implement the "open" function'); }),
            close: (_) => __awaiter(this, void 0, void 0, function* () { throw new Error('transport does not implement the "close" function'); }),
            send: (_) => __awaiter(this, void 0, void 0, function* () { throw new Error('transport does not implement the "send" function'); }),
        ...
        try {
            ...
            if ((0, utils_js_1.checkUrl)(server_config.getAddress()[0].url, ['ws', 'wss', 'ws+unix', 'wss+unix'])) {
                transport = new transport_ws_js_1.default(this);
            }
            if ((0, utils_js_1.checkUrl)(server_config.getAddress()[0].url, ['file'])) {
                transport = new transport_unix_js_1.default(this);
            }
            if (transport)
                this._transport = transport;
        }
        catch (error) {
            logger_js_1.default.error(`${LOG_NS} ${this.name} error while initializing transport (${error.message})`);
        }
    ...
    _signalClose(graceful) {
        ...
        if (graceful) {
             ...
             */
            this.emit(protocol_js_1.JANODE.EVENT.CONNECTION_CLOSED, { id: this.id });
        }
        else {
             ...
             * @type {Error}
             */
            this.emit(protocol_js_1.JANODE.EVENT.CONNECTION_ERROR, error);
        }
    ...
    _handleMessage(janus_message) {
        ...
        if (session_id && !this._config.isAdmin()) {
            ...
            if (!session) {
                logger_js_1.default.warn(`${LOG_NS} ${this.name} session ${session_id} not found for incoming message ${janus}`);
                return;
            }
            try {
                ...
                logger_js_1.default.error(`${LOG_NS} ${this.name} error while handling message (${error.message})`);
            }
            return;
        }
        ...
        if (transaction) {
            ...
            if (this._tm.getTransactionOwner(transaction) !== this) {
                logger_js_1.default.warn(`${LOG_NS} ${this.name} transaction ${transaction} not found for incoming messsage ${janus}`);
                return;
            }
            /*
        ...
        logger_js_1.default.error(`${LOG_NS} ${this.name} unexpected janus message directed to the connection ${JSON.stringify(janus_message)}`);
    }
    /**
    ...
    create(ka_interval) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} creating new session`);
            const request = {
                janus: protocol_js_1.JANUS.REQUEST.CREATE_SESSION,
            };
            try {
                const { data: { id } } = yield this.sendRequest(request);
                ...
                const session_instance = new session_js_1.default(this, id, ka_interval);
                ...
                session_instance.once(protocol_js_1.JANODE.EVENT.SESSION_DESTROYED, ({ id }) => {
                    this._sessions.delete(id);
                    this.setMaxListeners(this.getMaxListeners() - 2);
                });
                logger_js_1.default.info(`${LOG_NS} ${this.name} session created (id=${id})`);
                return session_instance;
            }
            catch (error) {
                logger_js_1.default.error(`${LOG_NS} ${this.name} session creation error (${error.message})`);
                throw error;
            }
        });
    }
    ...
    getInfo() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} requesting server info`);
            const request = {
                janus: protocol_js_1.JANUS.REQUEST.SERVER_INFO,
            };
    ...
    listSessions() {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.verbose(`${LOG_NS} ${this.name} requesting session list`);
            const request = {
                janus: protocol_js_1.JANUS.ADMIN.LIST_SESSIONS,
            };
            return this.sendRequest(request);
        });
    }
     ...
     * Logger.info(`${JSON.stringify(data)}`);
     */
    listHandles(session_id) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} requesting handle list`);
            if (!session_id) {
            ...
            const request = {
                janus: protocol_js_1.JANUS.ADMIN.LIST_HANDLES,
                session_id,
            };
            return this.sendRequest(request);
        });
     ...
     * Logger.info(`${JSON.stringify(data)}`);
     */
    handleInfo(session_id, handle_id) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} requesting handle info`);
            if (!session_id) {
                const error = new Error('session_id parameter not specified');
                logger_js_1.default.error(`${LOG_NS} ${this.name} ${error.message}`);
                throw error;
            ...
            const request = {
                janus: protocol_js_1.JANUS.ADMIN.HANDLE_INFO,
                session_id,
                handle_id,
            };
            return this.sendRequest(request);
     ...
     */
    startPcap(session_id, handle_id, folder, filename, truncate) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} requesting pcap start for handle ${handle_id}`);
            if (!session_id) {
                const error = new Error('session_id parameter not specified');
                logger_js_1.default.error(`${LOG_NS} ${this.name} ${error.message}`);
                throw error;
            ...
            const request = {
                janus: protocol_js_1.JANUS.ADMIN.START_PCAP,
                session_id,
                handle_id,
                folder,
                filename,
     ...
     */
    stopPcap(session_id, handle_id) {
        return __awaiter(this, void 0, void 0, function* () {
            logger_js_1.default.info(`${LOG_NS} ${this.name} requesting pcap stop for handle ${handle_id}`);
            if (!session_id) {
                const error = new Error('session_id parameter not specified');
                logger_js_1.default.error(`${LOG_NS} ${this.name} ${error.message}`);
                throw error;
            ...
            const request = {
                janus: protocol_js_1.JANUS.ADMIN.STOP_PCAP,
                session_id,
                handle_id,
            };
            return this.sendRequest(request);
